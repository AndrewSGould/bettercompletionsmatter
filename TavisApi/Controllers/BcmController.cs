  ...
  public IActionResult GetRandomGame(int playerId)
  {
    ...
    var TavisRandomGameOptions = _context.PlayerGames?
                    ...
                    .Where(x => x.PlayersGames.PlayerId == playerId 
                      ...
                      && TavisRule.RandomValidPlatforms.Contains(x.PlayersGames.Platform!))
                    .ToList();

    if (TavisRandomGameOptions?.Count() < TavisRule.RandomMinimumEligibilityCount) {
      return BadRequest($"{player.Name} did not have enough eligible games! " +
            $"Player needs to add {50 - TavisRandomGameOptions.Count()} more games.");
    }
    
    var rand = new Random();
    var randomGame = TavisRandomGameOptions?[rand.Next(TavisRandomGameOptions.Count())];

    return Ok(new {
      selectedGame = randomGame,
      potentials = TavisRandomGameOptions
    });
  }
    ...
    foreach(var player in players) {
      ...
      if (TavisRandomGameOptions?.Count() < TavisRule.RandomMinimumEligibilityCount) {
        playersIneligible.Add(new {
          Player = player.Name,
          EligibleCount = TavisRandomGameOptions.Count()
        });
      }
    }

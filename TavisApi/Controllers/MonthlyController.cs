...
public class MonthlyController : ControllerBase {
	...
	[HttpGet, Route("mar-leaderboard")]
	public async Task<IActionResult> GetMarLeaderboard()
	{
		var recap = await _context.MarRecap.ToListAsync();
		return Ok(recap);
	}
	...
	[HttpGet, Route("jun-leaderboard")]
	public async Task<IActionResult> GetJunLeaderboard()
	{
		var recap = await _context.JunRecap.ToListAsync();
		return Ok(recap);
	}

	[HttpGet, Route("jul-leaderboard")]
	public async Task<IActionResult> GetJulLeaderboard()
	{
		var recap = await _context.JulyRecap.ToListAsync();
		return Ok(recap);
	}
